jQuery(document).ready(function() {
	...
	if( !jQuery( ".opportune-info" ).length ) {
		...
		jQuery('.opportune-info').append('<a style="width: 80%; margin: 5px auto 5px auto; display: block; text-align: center;" href="https://www.shapedpixels.com/setup-opportune/" class="button" target="_blank">{setup}</a>'.replace('{setup}', opportuneCustomizerObject.setup));
		...
		jQuery('.opportune-info').append('<a style="width: 80%; margin: 5px auto 5px auto; display: block; text-align: center;" href="https://www.shapedpixels.com/opportune-pro" class="button" target="_blank">{pro}</a>'.replace('{pro}',opportuneCustomizerObject.pro));
